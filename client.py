...
def run_progressbar(self,i,N,flag,name):
	...
	#time.sleep(0.02)
	self.map2loop_log_TextEdit.append(str(name)+' is transferring ...............'+"\n")
	
	if i==N:
		if flag=='PC ------> Server':
			self.map2loop_log_TextEdit.append('............END MODE: '+str(flag)+ '.............. '+"\n")
			self.map2loop_progressBar.setVisible(False)
		elif flag=='Server ------> PC':
			#self.map2loop_log_TextEdit.setFontWeight(QFont.Bold)
			self.map2loop_log_TextEdit.append('ALL RESULT DATA HAVE SUCCESSFULLY BEEN TRANSFERRED TO YOUR LOCAL PC'+ "\n")
			self.map2loop_log_TextEdit.append('............END MODE: '+str(flag)+ '.............. '+"\n")
			self.map2loop_progressBar.setVisible(False)
		else:
			for i in range(5):
...
async def data_uploader(filename,filepath,idx,how_many_file_need_to_be_sent,ip_adress,port_number):
...
def client_main(self,userID,ip_adress,port_number,config_param,local_data_path):
	'''
	...
	try:
		...
		if self.ping_message[0]==1:
			...
			self.map2loop_log_TextEdit.setGeometry(170,150, 750, 300)
			self.map2loop_log_TextEdit.append('Successfully connected to the host:: '+ str(ip_adress)+"\n")
			time.sleep(0.05)
			...
			for file, filepath in zip(filename,list_of_data):
				#print('counter: ',idx, Nbre_data_sent)
				run_progressbar(self,idx,nbre_pc_data_to_server,outgoing_flag,file)
				all_data_uploaded_msg =asyncio.get_event_loop().run_until_complete(data_uploader(file,filepath,idx,nbre_pc_data_to_server,ip_adress,port_number))
				if idx==nbre_pc_data_to_server:
					self.map2loop_log_TextEdit.append(str(all_data_uploaded_msg[0])+ "\n")
				idx+=1

			map2loop_output,map2loop_execution_msg = asyncio.new_event_loop().run_until_complete(map2loop_executor(str(config_param),ip_adress,port_number))
