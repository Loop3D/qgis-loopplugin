...
def load_data_when_qgis_is_choosen(self, colname, geol_name, lyr):
    ...
    if (
        self.sender().objectName() == "Ok_pushButton"
        and geol_name == "GeolButton"
...
def switch_qpush_in_qgis_option(self, name):
    ...
    try:
        if name == "StructButton":
            ...
            if (
                self.Fault_checkBox.isChecked() == True
                and self.Geology_checkBox.isChecked() == True
            ):
